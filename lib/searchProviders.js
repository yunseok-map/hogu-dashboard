...
export function similarity(a, b) {
  ...
  // 모델명(영문+숫자 혼합 토큰) 일치 시 강한 보너스
  const models = [...A].filter((w) => /[a-z]/.test(w) && /\d/.test(w));
  if (models.some((m) => B.has(m))) score = Math.min(1, score + 0.35);
  // 부속품 오염 방지: 페널티 + 상한 캡 (모델명이 같아도 부속품은 비교군 문턱을 못 넘게)
