...
/** 흔한 카테고리/마케팅 토큰 — 상품을 구별하지 못하므로 유사도 가중치를 낮춘다. */
export const STOP = new Set(
  ('무선 유선 블루투스 정품 새제품 새상품 미개봉 국내 해외 병행 당일 발송 배송 관부가세 포함 옵션 색상 '
  + '블랙 화이트 실버 골드 그레이 그레이스 블루 레드 핑크 그린 퍼플 베이지 최신 공식 인증 강력 초경량 가정용 '
  + '이어폰 헤드폰 스피커 마우스 키보드 노트북 태블릿 세트 패키지 신형 신제품').split(' ')
);

/** 용량/크기/수량 등 "가격을 가르는 사양"을 정규화해 뽑는다 (128GB·27인치·500ml·2개 …) */
const SPEC_UNITS = [
  [/(\d+(?:\.\d+)?)\s?(?:tb|테라)\b/gi, 'tb'],
  [/(\d+(?:\.\d+)?)\s?(?:gb|기가)\b/gi, 'gb'],
  [/(\d+(?:\.\d+)?)\s?(?:ml|㎖)\b/gi, 'ml'],
  [/(\d+(?:\.\d+)?)\s?(?:l|리터)\b/gi, 'l'],
  [/(\d+(?:\.\d+)?)\s?(?:인치|형|inch)\b/gi, 'in'],
  [/(\d+(?:\.\d+)?)\s?(?:cm|센치|센티미?터?)\b/gi, 'cm'],
  [/(\d+(?:\.\d+)?)\s?(?:w|와트)\b/gi, 'w'],
  [/(\d+)\s?(?:개입|개|매|장|팩|구|병|포)\b/g, 'ea'],
];
export function extractSpecs(str) {
  const s = String(str || '').toLowerCase();
  const out = new Set();
  for (const [re, unit] of SPEC_UNITS) {
    for (const m of s.matchAll(re)) {
      const v = m[1].replace(/\.0$/, '');
      if (unit === 'ea' && v === '1') continue; // "1개"는 구별력 없음
      out.add(v + unit);
    }
  }
  return out;
...
export function similarity(a, b) {
  ...
  // 3) 스펙 불일치 → 다른 변형(가격대 다름)
  const sA = extractSpecs(a), sB = extractSpecs(b);
  if (sA.size && sB.size && ![...sA].some((s) => sB.has(s))) score *= 0.5;
