...
import { extractPromos } from './searchProviders.js';
...
/** 검색 쿼리용 상품명 정제: 몰 이름/광고 문구/옵션 꼬리표 제거 후 핵심 토큰만 남긴다. */
export function buildSearchQuery(title) {
  if (!title) return '';
  let t = title
    .replace(/\[[^\]]*\]|\([^)]*\)/g, ' ')            // [무료배송], (당일출고) 류 제거
    .replace(/(?::|\||-)\s*(?:네이버|쿠팡|11번가|G마켓|옥션|SSG|티몬|위메프|인터파크|다나와)[^,]*$/i, ' ')
    .replace(/(?:무료배송|당일발송|당일출고|정품|공식|특가|할인|세일|사은품|증정|기획전|모음전?|1\+1|BEST|HOT|NEW)/gi, ' ')
    .replace(/[^\w가-힣.\- ]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  // 토큰 수 제한 (모델명 같은 영숫자 토큰은 우선 보존)
  const tokens = t.split(' ').filter(Boolean);
  if (tokens.length > 7) {
    const modelish = tokens.filter((w) => /[a-z]/i.test(w) && /\d/.test(w));
    const rest = tokens.filter((w) => !modelish.includes(w));
    t = [...rest.slice(0, 7 - Math.min(modelish.length, 3)), ...modelish.slice(0, 3)].join(' ');
  }
  return t;
}
