...
- 파서가 잘 되는지: `node scripts/probe.mjs <URL>` (압축 요약 JSON만 출력, HTML 미노출)
- 검색이 잘 되는지: `node scripts/search-probe.mjs "<검색어>"` (제공자별 카운트+통계만)
...
## 5. 서버 재시작
lib/ 수정 후에는 서버 재시작 필요 (ESM 모듈 캐시). 브라우저 preview_stop → preview_start (launch.json의 "hogu" 설정 사용).

## 6. 파일 구조 (다시 탐색하지 말 것)
```
./
  server.js            Express + SSE, API 엔드포인트 전부 여기
  lib/fetchPage.js     브라우저 위장 fetch
  lib/productParser.js 상품 페이지 파싱 (JSON-LD/OG/사이트 어댑터)
  lib/searchProviders.js 네이버API·다나와·DDG 검색 + 유사도
  lib/verdict.js       호구지수 스코어링
  lib/store.js         히스토리 저장
  public/              프런트 (index.html, app.js, style.css)
  docs/DESIGN.md       UI 스펙 — UI 작업은 ui-work 스킬 로드 후 이것만 읽기
  scripts/             probe.mjs, search-probe.mjs, launch-chrome.mjs
```
