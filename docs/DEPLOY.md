...
- [ ] **자동 시작**(재부팅 후 자동 복구):
  - 서버: 작업 스케줄러에 "로그온 시 `node start-prod.mjs`" 등록, 또는 `nssm`/`pm2-windows`로 서비스화.
  - 터널: `cloudflared service install`(위 B3).
- [ ] **크래시 복구**: pm2 또는 nssm으로 죽으면 재시작.
...
- [ ] **히스토리 공개 범위 결정**: 현재 계정 개념이 없어 `기록(history)`이 **전역 공유**(방문자 모두가 서로의 검사 기록을 봄). 공개 서비스로 부적절하면 알려줘 → prod에서 히스토리 노출/기록을 끄는 옵션 추가해줌.
