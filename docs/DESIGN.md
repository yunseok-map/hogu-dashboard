...
- 팔레트: **네온 시트러스**(사용자 재선택, 크림톤 폐기) — 옅은 라일락 `--paper(#f6f5ff)` + 잉크 `--ink(#17141f)` + 바이올렛 메인 `--blue(#6d3bff)`(검사 CTA·링크·번호) + 옐로우 포인트 `--lime(#ffd43a)`(딜카드·하이라이트·배지). 티어색(t-great~t-hogu)은 브랜드와 무관한 의미 스케일이라 별도 유지. 배경은 라일락 + 방안지 도트그리드(html 직접, body 투명).
