...
- 라이브 페이지/검색 검증은 `scripts/probe.mjs`, `scripts/search-probe.mjs`만 사용.
...
- `lib/` 수정 후에는 서버 재시작 필요.

## 아키텍처 (탐색 대신 이 표 사용)
| 파일 | 역할 |
|---|---|
| server.js | Express, SSE 스트리밍 분석(`/api/analyze/stream`), POST `/api/analyze`, 히스토리 CRUD, `.env` 로더 |
| lib/fetchPage.js | 브라우저 헤더 위장 fetch + EUC-KR 처리 (1차) |
| lib/browserFetch.js | 봇 차단 시 2차: ①열린 CDP Chrome → ②서버가 실제 Chrome 자동 spawn(화면밖, 전용 프로필)+CDP → ③headless 폴백. 오리진 홈 방문 워밍업(Akamai 쿠키)+차단 감지 1회 재시도. 60s 유휴/서버종료 시 정리. `HOGU_NO_AUTO_CHROME=1`로 자동실행 차단 |
| lib/productParser.js | 1차 HTTP → 차단·가격누락 시 2차 브라우저 재시도. JSON-LD → OG → 사이트 어댑터(스마트스토어/쿠팡) → 인라인 JSON 순 파싱. 추출 품질 점수로 더 나은 결과 채택 |
| lib/searchProviders.js | 기본: 다나와+에누리 크롤링(키 불필요)+DuckDuckGo+네이버API(선택). 정밀(deep=1, UI 체크박스): SSG(__NEXT_DATA__ 딥스캔)+11번가(.c-card-item)+옥션+G마켓(searchEbayKorea 공용, **상품 링크 앵커**로 카드 잡음 — 옥션/G마켓 클래스가 서로 달라서). deep 제공자는 한 Chrome 공유라 **반드시 순차 실행**(동시 실행 시 렌더 방해로 0건). extractPromos로 쿠폰/이벤트 문구를 각 item.promos에 담음. 유사도 엔진(고도화): 가중 자카드(흔한 카테고리어 STOP는 가중치↓) + 모델코드 정규화 매칭(modelCodes, "SM-R630N"="SM R630N", 다른 모델이면 0.3 캡) + 스펙 인식(extractSpecs 용량/크기/수량 — 128GB↔256GB처럼 다르면 ×0.5 감점) + 부속품 3중 방어(ACC 어휘→0.15 캡→`it.accessory`로 verdict 비교군 원천 배제). buildSearchQuery도 동일 헬퍼로 핵심명사+모델+스펙 우선 조립. 중복 제거. **불가 확정(재시도 금지, 2026-07-18 실측)**: 네이버쇼핑(로그인+캡차), G마켓(실제 Chrome으로도 connection 안 열림 — goto 행 유발 주의), 삼성닷컴/LG몰(Chrome+스크롤 트리거로도 가격 미렌더 → 대신 브랜드 감지 시 공식몰 검색 링크를 reviewSearchLinks에 추가함) |
| lib/verdict.js | 호구지수(백분위50+중앙값프리미엄25+최저가과지불20±평점10), 등급/사유/추천대안/리뷰판단. **buildDealPitch**: 쿠폰 반영 실구매가(item.effPrice=price−promoDiscount) 최저 딜을 골라 "이래서 이득" 설득 문구 생성. 대안·savingsPotential도 effPrice 기준 |
| lib/store.js | data/history.json + data/results/{id}.json |
